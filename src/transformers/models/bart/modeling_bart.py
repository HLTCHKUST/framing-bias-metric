...
class BartForConditionalGenerationAndClassification(PretrainedBartModel):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [r"final_logits_bias", r"encoder\.version", r"decoder\.version"]
    ...
    def forward(
        ...
        return_dict=None,
        extra_task=None,
        extra_task_input_ids=None, 
        extra_task_attention_mask=None, 
        ...
        # TODO: check what these output attentions are. do i also need to configure these for our new task?
        extra_task_loss = None

        if extra_task in ['webis', 'propaganda', 'clickbait', 'lr_vs_roundup']: # extra CLASSIFICATION task
            
            # # previous version
            # bart_encoder = self.get_encoder()
            # extra_t1_encoder_outputs = bart_encoder(
            #         input_ids=extra_task_input_ids,
            #         attention_mask=extra_task_attention_mask,
            #         output_attentions=None,
            #         output_hidden_states=None,
            #         return_dict=return_dict,
            #     )

            # # print(extra_t1_encoder_outputs[0].shape) # torch.Size([1, 365, 1024])
            # # print(torch.mean(extra_t1_encoder_outputs[0], dim=1)) # torch.Size([1, 1024])
            # extra_t1_encoder_outputs_avg = torch.mean(extra_t1_encoder_outputs[0], dim=1) 
            # extra_t1_logits = self.classification_head(extra_t1_encoder_outputs_avg)

            # # additional t1 classification loss on the BART ``encoder''
            # loss_fct = CrossEntropyLoss()
            # extra_task_loss = loss_fct(extra_t1_logits, extra_task_label)


            # new version
            extra_task_encoder_outputs = self.model(
                input_ids=extra_task_input_ids,
                attention_mask=extra_task_attention_mask,
                return_dict=return_dict,
            )

            x = extra_task_encoder_outputs[0]  # last hidden state
            ...
            loss_fct = CrossEntropyLoss()
            extra_task_loss = loss_fct(extra_task_logits.view(-1, self.extra_task_num_labels), extra_task_label.view(-1))
        ...
        # note: LM loss is calculated outside this function - based onb the returned lm_logits.


        if not return_dict:
            output = (lm_logits,) + outputs[1:]
            return ((extra_task_loss,) + output) if masked_lm_loss is not None else output

        return Seq2SeqLMOutput(
            loss=extra_task_loss,
            logits=lm_logits,
            past_key_values=outputs.past_key_values,
            decoder_hidden_states=outputs.decoder_hidden_states,
            decoder_attentions=outputs.decoder_attentions,
